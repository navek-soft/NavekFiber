...
csapi_python::csapi_python(const core::coptions& options)
	...
	PyImport_AppendInittab("fiber", PyInit_fiber_module);
	Py_Initialize();
...
void csapi_python::OnPUT(csapi::request& msg) {
	...
	parameters.init(headers, msg.request_uri().trim().str(), headers.at({ "x-fiber-msg-id", 14 }).trim().str(), msg.request_paload());
