...
static inline size_t initRoutes(Fiber::RouterImpl& router, Dom::Client::Manager<IKernel>& kernel, 
	...
	for (auto&& ch : Channels) {
		...
		if (router.AddRoute(std::get<2>(ch.second), std::get<0>(mq), std::get<3>(ch.second).get(), Dom::Interface<IKernel>(&kernel))) {
			numChannels++;
