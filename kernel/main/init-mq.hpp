...
static inline size_t initMQ(const Fiber::ConfigImpl& options, Dom::Client::Manager<IKernel>& fiberRegistry,std::unordered_map<std::string, std::unique_ptr<const Fiber::ConfigImpl>>& ClassesMq) {
	...
	for (auto&& mq : options.GetSections()) {
		...
		try {
			if (!mq_class.empty() && !mq_module.empty() && fiberRegistry.EmplaceServer(mqPath + mq_module, "MQ")) {
				numMQ++;
				ClassesMq.emplace(mq_class, std::unique_ptr<const Fiber::ConfigImpl>(new Fiber::ConfigImpl(options[mq.c_str()])));
